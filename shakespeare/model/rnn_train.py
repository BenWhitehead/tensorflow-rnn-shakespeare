...
def train_and_evaluate(hparams):
    ...
    summary_writer = tf.summary.FileWriter(
        "{}/{}-training".format(hparams.log_dir, timestamp)
    )
    validation_writer = tf.summary.FileWriter(
        "{}/{}-validation".format(hparams.log_dir, timestamp)
    )

    # Init for saving models. They will be saved into `hparams.checkpoint_dir`.
    ...
    for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, nb_epochs=10):
        ...
        if step // 10 % _50_BATCHES == 0:
            saved_file = saver.save(
                sess,
                '{}/rnn_train_{}'.format(hparams.checkpoint_dir, timestamp),
                global_step=step
            )
